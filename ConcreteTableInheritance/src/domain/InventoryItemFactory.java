...
/**
 * A "factory pattern" enum for building InventoryItem objects Each type of
 * InventoryItem object that has a corresponding table in the db has an
 * enumeration here
 * 
 * @author mb8542
 *
...
public enum InventoryItemFactory implements FactoryInterface {
	Nail {
		...
		/**
		 * Builds a Nail, the fourth and fifth parameters, with respect to
		 * order, should be (integer) numberInBox and (double) length
		 */
		@Override
	...
	StripNails {
		...
		/**
		 * Builds a StripNail, the fourth and fifth parameters, with respect to
		 * order, should be (integer) numberInStrip and (double) length
		 */
		@Override
		public InventoryItem buildItem(String upc, int manufacturerId, int price, Object... objects) {
			staticTemporaryList.clear();
	...
	PowerTool {
		/**
		 * Builds a PowerTool, the fourth parameter should be an integer value
		 * corresponding to if the tool is battery-powered and the fifth
		 * parameter should be a string representing the description
		 */
		@Override
		public InventoryItem buildItem(String upc, int manufacturerId, int price, Object... objects) {
			staticTemporaryList.clear();
	...
	protected void addCommonParameters(String upc, int manufacturerId, int price) {
		staticTemporaryList.add(-27); // see ID_NOT_ISSUED in
